...
2. link：把原文件的inode_id拿到，new一个新name到旧id的dirent。unlink：首先找inode，删掉dirent，如果删掉的是最后的dirent就要先用clear函数删掉当前inode的数据，然后用dealloc_inode（EasyFileSystem.inode_bitmap.dealloc）删掉inode的位图。
3. bugfix：不知道为什么在spawn时如果new_task直接new出来会在第二次执行`ch5_spawn*`时`all_data`空掉（`app_inode.read_all();`读不了），但是先fork再`new_task.exec(all_data.as_slice());`就没问题？？？
