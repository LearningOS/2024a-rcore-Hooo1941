...
use crate::{
    ...
    mm::{copy_to_user, translated_refmut, translated_str, MapPermission},
    task::{
        add_task, current_task, current_user_token, exit_current_and_run_next,
        get_curr_task_status, mmap, munmap, suspend_current_and_run_next, TaskControlBlock,
...
pub fn sys_spawn(_path: *const u8) -> isize {
    ...
    if let Some(app_inode) = open_file(path.as_str(), OpenFlags::RDONLY) {
        let all_data = app_inode.read_all();
        let new_task = TaskControlBlock::new(all_data.as_slice());
        let new_task = Arc::new(new_task);
        new_task.inner_exclusive_access().parent = Some(Arc::downgrade(&current_task().unwrap()));
        let new_pid = new_task.pid.0;
        // info!("kernel:pid[{}] sys_spawn[{}]", current_task().unwrap().pid.0, new_pid);
        current_task()
            .unwrap()
...
pub fn sys_set_priority(_prio: isize) -> isize {
    ...
    current_task().unwrap().inner_exclusive_access().pass = BIG_STRIDE / _prio as usize;
    _prio
}
